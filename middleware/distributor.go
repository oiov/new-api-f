...
// 修复 #4834: GET /v1/video/generations/:task_id 与 /v1/videos/:task_id 此前不解析 model，
// 当 token 启用「可用模型限制」时，下游 modelLimitEnable 校验会因 modelRequest.Model
// 为空而误报 "This token has no access to model"。从已存储的任务记录中回填
// OriginModelName 即可让校验走在正确的模型上。仅在启用模型限制时做只读 DB 查询。
