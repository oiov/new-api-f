...
> Scope: 在 `web-worker` 的 `/console/leaderboard` 天梯榜中，新增一列「分析」，估计每个头部账号在所选时间段内「疑似几个人在使用」，点击整行弹出右侧抽屉展示证据明细；把天梯榜的日期筛选从「按天」升级为「自定义时间段」（默认当天）；并把榜单从固定前 20 名升级为**分页浏览范围内全部有日志的用户 + 按 user_id/用户名搜索**。
...
- **翻页 + 搜索**：不再只看前 20 名，而是分页浏览筛选范围内**所有有消费日志的用户**；提供搜索框，按 user_id / 用户名查任意用户的分析统计。
- 所有相关接口**仅管理员**可访问。
...
2. **地理分布 `distinct_geo`** — `distinct_geo` = distinct `(country, city)` 对的数量；同一国家内所有空城市行归并为单个 `(country, "")`。跨国 > 跨城 > 同城。`city==""` 渲染见 §6.5（只显示国家）。
...
// 列表级「疑似中转」廉价代理（不需要并发信号；仅 has_ip_data 时有效）：
list_transit_suspected = has_ip_data && (subnet_clusters >= 1 && subnet_clusters <= 2) && (request_count >= T_LIST)   // T_LIST 默认 200
...
- **`subnet_clusters == 0`（无 IP 记录）不算「集中」**：中转判定要求确有集中的少量 IP（`>= 1`），避免把「没记 IP」误判成「IP 集中」（见 §3.4）。
- 抽屉里的 §3.2 是**独立、更强**的中转判定，可能得出更高的估计（见 §3.6 口径对齐）。

### 3.2 中转分发识别（抽屉接口内计算）

**前置门控**：仅当 `has_ip_data == true`（即 `subnet_clusters >= 1`）才进行中转判定。`has_ip_data == false` 时**跳过本节**，走 §3.4 退化路径（不得输出「IP 高度集中」类结论）。

信号组合：`subnet_clusters >= 1 && subnet_clusters <= 2`（IP 确有且高度集中）**且** `max_concurrency >= K`（默认 K=3）**且** `input_dispersion_score >= 2`（见 §3.2.1）。
...
- **已知边界（非 bug）**：桶为固定数量级边界，相邻边界两侧的相近值（如并发的 900 与 1100）会落入不同桶而使 `score=2`，尽管仅 ~1.2× 差距。作为启发式可接受，审查时勿误判为缺陷。

### 3.3 置信度（high / medium / low）

置信度分**列表档（廉价信号）**与**抽屉档（全信号）**两套，因为列表拿不到并发（§3.1）：

**抽屉档（全信号，用于 `LeaderboardAnalysisDetail.Confidence`）**
...
**列表档（仅便宜信号，用于 `LeaderboardAnalysis.Confidence`，去掉一切并发项）**

| 置信度 | 判据 |
|---|---|
| **high** | 多国/多城 **且** 多 token。 |
| **medium** | 多 IP 同国，或有 token 分叉但地理单一。 |
| **low** | 数据不足：IP 全空、或请求数过少（< 10）、或 GeoIP 缺失。 |
...
- `p`、`page_size` —— 分页，沿用 `common.GetPageQuery(c)`（默认 `ItemsPerPage`，上限 100；参数名 `p` + `page_size`）。
- `keyword` —— 用户搜索（见 §5.1a）。
- 保留既有 `date`（YYYY-MM-DD）与 `sort_by`。
- 优先级：给了 `start_timestamp`/`end_timestamp` 用之；否则回落 `date`；再否则默认当天 00:00 → 现在（`end=0` 表示不设上界，沿用既有约定）。

> **语义变化（非 top-20）**：接口不再固定返回前 20 名，而是**分页返回筛选范围内所有有消费日志的用户**（仍按 `sort_by` 降序）。`total = COUNT(DISTINCT user_id)`（跨库安全）。第 1 页前 3 名仍供前端做 podium。
...
type LeaderboardAnalysis struct {
    ...
    Confidence   string           `json:"confidence"`      // high|medium|low（列表档，仅便宜信号，见 §3.3）
    IpCount      int              `json:"ip_count"`        // 去重子网簇数
    TokenCount   int              `json:"token_count"`     // 去重活跃 token 数
    TopRegions   []LeaderboardRegion `json:"top_regions"`  // 最多 3 个，按 count 降序
...
**`LeaderboardResponse` 新增分页字段**（保留既有 `summary` / `leaderboard`）：
...
- `total`：`SELECT COUNT(DISTINCT user_id) ... WHERE 同筛选（含 keyword 候选）`。
- 子网归并（/24、/48）+ GeoIP 解析 + `distinct_geo` 空城市归并均在 **Go 层**做（不进 SQL，规避跨库函数差异，符合根 Rule 2）。
- **`summary` 计算独立于分页**：`LeaderboardSummary`（全站总量 + top-N 占比）沿用既有逻辑，其 `Top10*` 子合计从**独立的 top-N 聚合**（如 top 20，与 `page_size` 无关）算出，**不**从当页 entries 求和。仅当 `p==1 && keyword==""` 时计算并返回（其余情况可省略以省一次全站聚合）。
...
- `useLeaderboard` hook 的 queryKey 纳入 `p`/`page_size`/`keyword`。
- 均为 GET 读取，无写操作，不涉及 §Rule 7 的高危写接口对齐（但仍遵守下划线字段名 + 全 200 约定）。

### 6.2a 页面状态与路由（`src/routes/console/leaderboard.tsx`）

- 新增 URL search 参数：`p`（默认 1）、`page_size`、`keyword`、起止时间戳、`sort_by`。分页/搜索/时间/排序均入 URL，可分享可刷新（参考日志页 `updateSearch` 模式）。
- 切换 tab（quota/tokens）、改时间、搜索 → 重置 `p=1`。
...
```
列表加载:
  前端 → GET /log/leaderboard?start_timestamp&end_timestamp&sort_by&p&page_size&keyword
       → 后端: 主聚合(quota/tokens/count) GROUP BY user_id ORDER BY <sort> LIMIT/OFFSET → 当页 user_id
             + total = COUNT(DISTINCT user_id)
             + 便宜信号(distinct ip / distinct token, 限当页 user_id)
             + [仅 p==1 && 无 keyword] summary(独立 top-N 聚合)
       → Go 层: 子网归并 + GeoIP 解析 + 估计算法(纯函数)
       → 返回 { summary?, leaderboard[].analysis, page, page_size, total }
  ⇒ 不扫原始行; 分析只对当页 ≤page_size 用户算, 宽窗口/深翻页也快

搜索:
  前端 keyword → 后端在 DB.users 解析候选 user_id(精确 id / 用户名 LIKE) → 日志聚合加 user_id IN 候选
  ⇒ 命中用户按 sort 排序分页返回, 各带分析列
...
- **列表接口**：绝不扫单用户全部原始行；主聚合分页取当页 user_id，便宜信号仅限当页 ≤page_size 用户。
...
2. **切片 2 — 后端估计算法（纯函数）+ 聚合查询**：信号抽取、子网归并、`EstimateAccountSharing` 纯函数、置信度（列表档/抽屉档两套）。纯函数单测优先（TDD）。
3. **切片 3 — 扩展 `/log/leaderboard`（含分页 + 搜索）**：start/end 参数、`p`/`page_size`（`common.GetPageQuery`）、`keyword` 搜索（DB.users 解析候选）、主聚合分页取当页 user_id、`total`、便宜信号限当页、`analysis` 字段拼装、best-effort geo、summary 独立计算且仅 p1 无搜索时返回。
4. **切片 4 — 新增 `/log/leaderboard/analysis`**：明细计算（并发扫描线、输入分桶、时间线、中转识别）、5 万采样边界、路由注册。
5. **切片 5 — 前端日期范围筛选 + 页面状态**：替换单日 Calendar 为起止选择，默认当天，快捷按钮；URL search 参数（时间/排序/`p`/`page_size`/`keyword`），改条件重置 `p=1`。
6. **切片 6 — 前端分析列 + 分页 + 搜索 + api-client**：types/hook（含分页字段）、列表「分析」列、全局名次、搜索框、分页控件、podium/summary 仅 p1 无搜索时显示、podium 徽标。
...
- **后端（优先 TDD）**：
  ...
  - 分页 + 搜索：`total = COUNT(DISTINCT user_id)` 正确、当页 user_id 取 LIMIT/OFFSET 正确、`keyword` 纯数字走 user_id 精确+用户名、非数字走用户名 LIKE、命中为空返回空表、summary 仅 p1 无搜索时返回。
- **前端**：
