  ...
  return (
    ...
    >
      <div className='flex flex-col gap-4 p-4 md:p-5'>
        ...
        >
          <div className='flex flex-col gap-3'>
            ...
            <div>
              <Typography.Text
                strong
                style={{ display: 'block', fontSize: 22, lineHeight: '30px', color: '#0f172a' }}
              >
                {currentProviderName}
              </Typography.Text>
              {inferredApp ? (
        ...
        <div style={panelCardStyle}>
          <Typography.Text strong style={{ display: 'block', marginBottom: 10, fontSize: 15 }}>
            {t('官方链接')}
          </Typography.Text>
          ...
          <Typography.Text type='tertiary' style={{ display: 'block', marginTop: 10 }}>
            {t('导入前会检测本地是否已安装 CC Switch；模型测试会发起一次最小化请求，用于校验当前模型和令牌是否可用。')}
          </Typography.Text>
        </div>
        ...
        <div style={panelCardStyle}>
          {currentConfig.modelFields.map((field, index) => (
            <div
              key={field.key}
              style={{
                paddingBottom: index === currentConfig.modelFields.length - 1 ? 0 : 18,
                marginBottom: index === currentConfig.modelFields.length - 1 ? 0 : 18,
                borderBottom:
