...
function deriveResetCustomFields(seconds) {
  ...
  if (value >= 86400 && value % 86400 === 0) {
    return {
      quota_reset_custom_value: value / 86400,
      quota_reset_custom_unit: 'day',
    };
  }
  if (value >= 3600 && value % 3600 === 0) {
    return {
      quota_reset_custom_value: value / 3600,
      quota_reset_custom_unit: 'hour',
    };
  }
  if (value >= 60 && value % 60 === 0) {
    return {
      quota_reset_custom_value: value / 60,
      quota_reset_custom_unit: 'minute',
    };
  }
  return {
    quota_reset_custom_value: Math.max(value, 1),
    quota_reset_custom_unit: 'second',
  };
}
...
const CLAUDE_MANUAL_DELIVERY_QUERY_URL = 'https://api-key-tool.nbility.dev/';
                  ...
                  <Card className='!rounded-2xl shadow-sm border-0 mb-4'>
                    <div className='flex items-center mb-2'>
                      ...
                      <div>
                        ...
                        <div className='text-xs text-gray-600'>
                          {t(
                            '支持自动开通站内订阅，或改为付款后由管理员人工发放结构化信息',
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <Card className='!rounded-2xl shadow-sm border-0 mb-4'>
                      ...
                      <Col span={12}>
                        {values.quota_reset_period === 'custom' ? (
                          ...
                          <div className='space-y-2'>
                            ...
                            {values.quota_reset_timing_mode ===
                            'fixed_clock' ? (
                              <div className='space-y-2'>
                                <Text>{t('固定重置时刻')}</Text>
                    ...
                    {hasResetWindow && (
                      <div className='mt-3 rounded-xl bg-blue-50 px-3 py-2 text-xs text-blue-700 dark:bg-blue-500/10 dark:text-blue-200'>
                        {t('重置预览')}：
                        {formatSubscriptionResetHint(resetPreviewPlan, t)}
                      </div>
                    )}
                  </Card>
