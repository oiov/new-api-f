...
const localizeTemplateOperations = (operations = [], t) =>
  (operations || []).map((operation) => {
    if (!operation || typeof operation !== 'object') {
      return operation;
    }
    const nextOperation = { ...operation };
    if (typeof nextOperation.description === 'string') {
      nextOperation.description = t(nextOperation.description);
    }
    if (Array.isArray(nextOperation.conditions)) {
      nextOperation.conditions = nextOperation.conditions.map((condition) =>
...
const ParamOverrideEditorModal = ({ visible, value, onSave, onCancel }) => {
  ...
  const operationModeLabelMap = useMemo(
    () =>
      OPERATION_MODE_OPTIONS.reduce((acc, item) => {
  ...
  const fillOperationsTemplate = (operationsPayload) => {
    const localizedPayload = localizeTemplateOperations(
      operationsPayload || [],
      t,
    );
    const nextOperations = localizedPayload.map(normalizeOperation);
    ...
    setExpandedConditionMap({});
    setJsonText(JSON.stringify({ operations: localizedPayload }, null, 2));
    setJsonError('');
    setEditMode('visual');
  };
  ...
  const appendOperationsTemplate = (operationsPayload) => {
    const appended = localizeTemplateOperations(
      operationsPayload || [],
      t,
