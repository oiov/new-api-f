...
function maskEmail(value) {
  ...
  return `${maskedLocal}@nbility.dev`;
}
...
function getBatchFailureMessages(results, recordsById, t, renderLabel) {
  return results
    .map((item, index) => {
      ...
      const label =
        typeof renderLabel === 'function' ? renderLabel(rawLabel) : rawLabel;
...
function renderBatchFailureDialog({ title, messages, t, handleCopy }) {
...
const EcomAgentPage = () => {
  ...
  const channelOptions = useMemo(() => {
    return (channels || []).map((channel) => {
      const extra = [channel.name, channel.tag, channel.group]
        .filter(Boolean)
        .join(' / ');
      return {
        label: extra ? `#${channel.id} · ${extra}` : `#${channel.id}`,
  ...
  const selectedManualReservedChannelId =
    getManualOrderReservedChannelId(selectedManualOrder);
  const selectedManualReservedChannelKeyIndex =
    getManualOrderReservedChannelKeyIndex(selectedManualOrder);
  ...
  const assignedChannelKeyOptions = useMemo(() => {
    ...
    return Array.from({ length: total }, (_, index) => {
      const isReservedForCurrentOrder =
        ...
        String(selectedAssignedChannel.id) ===
          String(selectedManualReservedChannelId) &&
  ...
  useEffect(() => {
    ...
    if (
      ...
      !assignedChannelKeyOptions.some(
        (item) =>
          item.value === String(assignmentForm.assigned_channel_key_index),
      )
    ) {
      setAssignmentForm((prev) => ({
        ...prev,
  ...
  const filteredAccounts = useMemo(() => {
    ...
    const normalizedAssignedUserKeyword = assignedUserKeyword
      .trim()
      .toLowerCase();
    return accounts.filter((record) => {
      ...
      const matchesStatus =
        !statusFilter || String(record.status || '') === statusFilter;
  ...
  const openEdit = async (record) => {
    ...
    try {
      const res = await API.post(
        `/api/ecomagent/accounts/${record.id}/edit`,
        {},
        {
          skipErrorHandler: true,
        },
    ...
    try {
      ...
      if (form.access_token_expires_at !== '') {
        payload.access_token_expires_at =
          Number(form.access_token_expires_at) || 0;
      }
  ...
  const handleDeliverLinkedOrder = async () => {
    ...
    try {
      ...
      showError(
        error?.response?.data?.message || error?.message || t('发放失败'),
      );
    } finally {
  ...
  const handleBatchSync = async () => {
    ...
    try {
      ...
      if (failed > 0) {
        showError(
          t('批量同步完成，成功 {{success}} 个，失败 {{failed}} 个。', {
            success,
            failed,
          }),
        );
            ...
            <div>
              ...
              <div className='mt-2 rounded-lg border border-[var(--semi-color-border)] px-3 py-2'>
                <div className='flex items-center gap-2 flex-wrap'>
                  <Text
                    ellipsis={{ showTooltip: true }}
                    style={{ maxWidth: isMobile ? 240 : 360 }}
                  >
                  ...
                  {record.api_key ? (
                    <Button
                      ...
                      theme='light'
                      onClick={() =>
                        handleCopy(record.api_key, t('API Key 已复制'))
                      }
                    >
                      {t('复制')}
                    </Button>
    ...
    <>
      ...
      >
        <div className='flex flex-col gap-3'>
          <Text type='tertiary'>
            {t(
              '账号密码和登录态二选一；如果你已有登录后的 session JSON，可直接粘贴导入。',
            )}
          </Text>
                ...
                <div className='flex flex-col gap-3'>
                  ...
                  {loginMode === 'token' ? (
                    <div className='flex flex-col gap-3'>
                      <Text size='small' type='tertiary'>
                        {t(
                          '适合你已经拿到 accountId、access_token、refresh_token 的场景。',
                        )}
                      </Text>
                      <Input
                        value={form.email}
                  ...
                  {loginMode === 'session' ? (
                    <div className='flex flex-col gap-3'>
                      <Text size='small' type='tertiary'>
                        {t(
                          '直接粘贴登录返回 JSON，会自动提取 email、accountId、access_token、refresh_token。',
                        )}
                      </Text>
                      <TextArea
                  ...
                  <Input
                    ...
                    placeholder={t(
                      '代理 URL，可选，支持 http(s):// 或 socks5://',
                    )}
                  />
                  <Text size='small' type='tertiary'>
                    {t(
                      '配置后，该账号的注册、登录、刷新 token、拉取订阅和生成 API Key 都会通过此代理发出。',
                    )}
                  </Text>
                  <Input
          ...
          <div className='flex justify-end gap-2 flex-wrap'>
            {detailRecord ? (
              <>
                ...
                <Button onClick={() => openEdit(detailRecord)}>
                  {t('编辑')}
                </Button>
                <Tooltip content={t('分配/发放')}>
                  <Button
          ...
          <div className='flex flex-col gap-4'>
            <div className='flex items-center justify-between gap-2 flex-wrap rounded-lg border border-[var(--semi-color-border)] p-3 bg-[var(--semi-color-fill-0)]'>
              <div className='min-w-0'>
                ...
                <div className='mt-1'>
                  <Text size='small' type='tertiary'>
                    {getAssignedOrderLabel(detailRecord, t)} /{' '}
                    {getAssignedChannelLabel(detailRecord, t)}
                  </Text>
                </div>
              </div>
            ...
            <div className='grid grid-cols-1 gap-3'>
              ...
              <div>
                ...
                <div>
                  {renderEmail(detailRecord.email)} /{' '}
                  {detailRecord.account_id || '-'}
                </div>
              </div>
              <div>
                ...
                <div>
                  {detailRecord.account_id ||
                    getSubscriptionData(detailRecord)?.accountId ||
                    '-'}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div>
                  {getAssignmentStatusLabel(detailRecord.assignment_status, t)}
                </div>
              </div>
              <div>
            ...
            <div className='rounded-lg border border-[var(--semi-color-border)] p-3'>
              ...
              <div className='mt-2 flex flex-col gap-1'>
                ...
                <Text size='small' type='tertiary'>
                  {t('Key 名称')}:{' '}
                  {getSubscriptionData(detailRecord)?.apiKeyName || '-'}
                </Text>
                <Text size='small' type='tertiary'>
            ...
            <div className='rounded-lg border border-[var(--semi-color-border)] p-3'>
              ...
              <div className='mt-2 flex flex-col gap-2'>
                <Text size='small'>
                  {t('密码')}:{' '}
                  {detailRecord.has_password ? t('已保存') : t('未保存')}
                </Text>
                <Text size='small'>
                  refresh:{' '}
                  {credentialLabel(
                  ...
                  {detailRecord.api_key ? (
                    <div className='flex items-center gap-2 flex-wrap'>
                      ...
                      <Button
                        ...
                        theme='light'
                        onClick={() =>
                          handleCopy(detailRecord.api_key, t('API Key 已复制'))
                        }
                      >
                        {t('复制')}
                      </Button>
                  ...
                  {getRecentLogs(detailRecord).length > 0 ? (
                    getRecentLogs(detailRecord).map((item, index) => (
                      <Text
                        ...
                        size='small'
                      >
                        {formatShanghaiDateTime(item?.timestamp)} ·{' '}
                        {item?.model || '-'} · {t('状态')} {item?.status || '-'}{' '}
      ...
      <SideSheet
        ...
        footer={
          <div className='flex justify-end gap-2 flex-wrap'>
            <Button onClick={() => setAssignmentRecord(null)}>
              {t('关闭')}
            </Button>
          ...
          <div className='flex flex-col gap-4'>
            <div className='rounded-lg border border-[var(--semi-color-border)] p-3 bg-[var(--semi-color-fill-0)]'>
              ...
              <div className='mt-1'>
                <Text size='small' type='tertiary'>
                  {t('accountId')}:{' '}
                  {assignmentRecord.account_id ||
                    getSubscriptionData(assignmentRecord)?.accountId ||
                    '-'}
                </Text>
              </div>
              <div className='mt-1'>
              ...
              <div className='mt-1'>
                <Text size='small' type='tertiary'>
                  ...
                  {' · '}
                  {t('绑定邮箱')}:{' '}
                  {renderEmail(assignmentRecord.assigned_user_email)}
                </Text>
              </div>
              <div className='mt-3 grid grid-cols-2 gap-3 md:grid-cols-4'>
                <div>
                  ...
                  <div className='mt-1'>
                    {getPlanLabel(assignmentRecord.plan)}
                  </div>
                </div>
                <div>
                  <Text size='small' type='tertiary'>
                ...
                <div>
                  ...
                  <div className='mt-1'>
                    {getRequestLimitLabel(assignmentRecord)}
                  </div>
                </div>
                <div>
                  <Text size='small' type='tertiary'>
                  ...
                  <div className='mt-1'>
                    {getUsedRequests(assignmentRecord)}
                  </div>
                </div>
                <div>
                  <Text size='small' type='tertiary'>
                  ...
                  <div className='mt-1'>
                    {getRequestRemain(assignmentRecord)}
                  </div>
                </div>
                <div>
                  <Text size='small' type='tertiary'>
                  ...
                  <div className='mt-1'>
                    {getTokenLimitLabel(assignmentRecord, t)}
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className='mt-3 flex flex-col gap-3'>
                ...
                <Select
                  ...
                  onChange={(value) => {
                    ...
                    const reservedChannelId =
                      getManualOrderReservedChannelId(selectedOrder);
                ...
                {selectedManualOrder ? (
                  <div className='rounded-lg border border-[var(--semi-color-border)] bg-[var(--semi-color-fill-0)] p-3'>
                    ...
                    <div className='mt-2 flex flex-col gap-2'>
                      <div>
                        <Text>
                          {selectedManualOrder?.order?.plan_title ||
                            selectedManualPlan?.title ||
                            '-'}
                        </Text>
                        ...
                        data={[
                          {
                            ...
                            value: `#${selectedManualOrder?.order?.id || '-'} · ${
                              ...
                              0
                                ? formatUidLabel(
                                    Number(
                                      selectedManualOrder?.order?.user_id || 0,
                                    ),
                                    t,
                                  )
                                : t('UID -')
                          ...
                          },
                          {
                            key: 'benefit',
                            label: t('权益'),
                            value: formatManualDeliveryBenefit(
                              selectedManualPlan,
                              t,
                            ),
                          },
                ...
                {selectedAssignedChannel ? (
                  <div className='flex flex-col gap-1'>
                    <Text size='small' type='tertiary'>
                      {t('已读取渠道配置')}:{' '}
                      {selectedAssignedChannel.name ||
                        `#${selectedAssignedChannel.id}`}
                      {' · '}
                      {selectedAssignedChannel.channel_info?.is_multi_key
                        ? t('多 Key {{count}} 个', {
                            count: Number(
                              selectedAssignedChannel.channel_info
                                ?.multi_key_size || 0,
                            ),
                          })
                        : t('单 Key')}
                ...
                <Text size='small' type='tertiary'>
                  {t(
                    '关联订单后，可直接调用人工发放订单 API，用当前账号 API Key 完成发放并自动回写绑定信息。',
                  )}
                </Text>
                <TextArea
          ...
          <div className='flex flex-col gap-4'>
            <div>
              ...
              <div className='mt-1'>
                <Text type='tertiary'>
                  {t('accountId')}:{' '}
                  {historyRecord.account_id ||
                    getSubscriptionData(historyRecord)?.accountId ||
                    '-'}
                </Text>
              </div>
            </div>
                ...
                <div className='flex items-center gap-2 flex-wrap'>
                  ...
                  {selectedRowKeys.length > 0 ? (
                    <>
                      ...
                      >
                        <Button loading={batchSyncing} disabled={batchDeleting}>
