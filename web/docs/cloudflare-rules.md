# Cloudflare Rules for FishXCode / new-api

## 适用架构

- `www.fishxcode.com` -> Cloudflare -> Vercel
- `api.fishxcode.com` -> Cloudflare -> Zeabur -> `new-api`
...
- 对应请求示例：
  - `https://fishxcode.com/api/user/checkin?month=2026-04`
  - `https://fishxcode.com/api/user/checkin?month=2026-05`

```cf
(
  lower(http.host) eq "fishxcode.com"
  or lower(http.host) eq "www.fishxcode.com"
)
...
- 如果 `api.fishxcode.com` 经常出现长请求超时，不要继续靠 WAF 规则硬扛
- 优先改：
  - 流式响应
  - 异步任务
  - 轮询结果
