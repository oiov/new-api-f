...
func TestLogsCanFilterAndExportByBusinessGroup(t *testing.T) {
	withLogStatTestDB(t, func() {
		...
		stat, err := SumUsedQuota(0, now-10, now+10, 10, "", "alice", "", 0, "vip", "研发部", "", "", "", 0, 0)
		require.NoError(t, err)
		require.EqualValues(t, 100, stat.Quota)
		require.EqualValues(t, 1, stat.RequestCount)

		exported, total, truncated, err := GetUserLogsForExport(10, LogTypeConsume, now-10, now+10, "", "", "", "研发部", "", "", "", 0, 0, false, true)
		require.NoError(t, err)
