...
func TestOaiResponsesStreamHandlerFallsBackToEstimatedPromptTokensWhenUpstreamUsageIsZero(t *testing.T) {
...
func TestOaiResponsesHandlerFallsBackToEstimatedPromptTokensWhenUpstreamUsageIsZero(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
	...
	require.Nil(t, apiErr)
	require.NotNil(t, usage)
	require.Equal(t, 3456, usage.PromptTokens)
	require.Equal(t, 0, usage.CompletionTokens)
	require.Equal(t, 3456, usage.TotalTokens)
}
...
func TestOaiResponsesToChatStreamHandlerFallsBackToEstimatedPromptTokensWhenUpstreamUsageIsZero(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	...
	require.Equal(t, 0, usage.CompletionTokens)
	require.Equal(t, 2345, usage.TotalTokens)
	require.True(t, info.HasSendResponse())
}
