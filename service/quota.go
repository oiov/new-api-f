...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	if totalTokens == 0 {
		...
		model.UpdateChannelUsedQuotaByKey(relayInfo.ChannelId, relayInfo.ChannelMultiKeyIndex, quota)
...
func PreConsumeTokenQuota(relayInfo *relaycommon.RelayInfo, quota int) error {
	...
	if err := model.CheckTokenPeriodQuota(token); err != nil {
		return err
	}
...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	if relayInfo != nil && relayInfo.BillingSource == BillingSourceSubscription {
		...
		delta := int64(quota)
		if delta != 0 {
			if err := model.PostConsumeUserSubscriptionDeltaForWindow(
				relayInfo.SubscriptionId,
				delta,
				relayInfo.SubscriptionResetWindowStart,
				relayInfo.SubscriptionResetWindowEnd,
			); err != nil {
				return err
			}
