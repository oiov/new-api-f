...
export function CheckinCard() {
  ...
  const checkedDates = new Set(historyStats?.records?.map((r) => r.checkin_date) ?? []);
  const recordMap = new Map(historyStats?.records?.map((r) => [r.checkin_date, r.quota_awarded]) ?? []);
    ...
    return (
      <Card className="card-pro">
        <CardContent className="p-5 flex items-center gap-3">
          ...
          <span className="text-sm text-muted-foreground">{t('加载签到状态...')}</span>
        </CardContent>
      </Card>
    );
  }
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="hidden sm:flex items-center gap-5 shrink-0">
              <div className="text-center">
                <p className="text-lg font-bold text-foreground">{stats?.checkin_count ?? 0}</p>
                <p className="text-[10px] text-muted-foreground">{t('本月签到')}</p>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-muted-foreground">{t('累计获得')}</p>
              </div>
            </div>
                ...
                <SheetContent side="right" className="w-[360px] sm:w-[420px] overflow-y-auto">
                  ...
                  <div className="mt-5 space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium">{getMonthLabel(historyMonth)}</span>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="size-8"
                      ...
                      <>
                          ...
                          <div className="grid grid-cols-7 gap-1">
                            ...
                            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                              ...
                              return (
                                <div
                                  key={day}
                                  title={quota !== undefined ? `+${formatQuota(quota, status)}` : undefined}
                                  className={[
                        ...
                        <div className="rounded-lg bg-muted/40 p-3 space-y-2">
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-muted-foreground">{t('本月签到天数')}</span>
                            <span className="font-medium">
                              {historyStats?.checkin_count ?? 0} {t('天')}
                            </span>
                          </div>
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-muted-foreground">{t('累计签到天数')}</span>
                            <span className="font-medium">
                              {historyStats?.total_checkins ?? 0} {t('天')}
                            </span>
                          </div>
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-muted-foreground">{t('累计获得额度')}</span>
                            <span className="font-medium text-primary">
                              {formatQuota(historyStats?.total_quota ?? 0, status)}
                            </span>
                          </div>
                        </div>

                        {/* Records list */}
                        {historyStats?.records && historyStats.records.length > 0 ? (
                          <div className="space-y-1">
                            ...
                            {historyStats.records.map((r) => (
                              ...
                              >
                                <span className="text-muted-foreground">{r.checkin_date}</span>
                                <span className="font-medium text-primary">
