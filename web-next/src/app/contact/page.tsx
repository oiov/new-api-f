...
const CONTACT_CARDS: ContactCard[] = [
  ...
  },
  {
    ...
    imageSrc: '/contact/fishxcode_user.jpg',
    value: 'fishxcode',
    copyValue: 'fishxcode',
  ...
  },
  {
    ...
    imageSrc: '/contact/wechat_group.jpg',
    icon: QrCode,
    color: {
    ...
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-8 space-y-10">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-xs font-semibold text-primary uppercase tracking-widest">{t('官方联系通道')}</span>
            <h1 className="text-2xl font-bold tracking-tight">{t('联系我们')}</h1>
          </div>
        </div>

        <p className="text-muted-foreground max-w-xl leading-relaxed">
          {t('欢迎加入社区或联系官方客服，选择最适合你的渠道，我们把常用入口整理在一个页面里。')}
        </p>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {HIGHLIGHTS.map((item) => {
            ...
            return (
              <div key={item.key} className="flex gap-3 p-3.5 rounded-xl bg-muted/40 border border-border/50">
        ...
        {CONTACT_CARDS.map((card) => {
          ...
          return (
            <motion.div key={card.key} variants={itemVariants}>
              <Card className={cn('border shadow-card h-full', card.color.border)}>
                <CardContent className="p-5 flex flex-col gap-4 h-full">
                  {/* 头部 */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className={cn('size-9 rounded-lg flex items-center justify-center shrink-0', card.color.bg)}>
                        <Icon className={cn('size-4', card.color.text)} />
                      </div>
                      <div>
                        <span className={cn('text-[10px] font-semibold px-1.5 py-0.5 rounded-full', card.color.badge)}>
                          {t(card.toneKey)}
                        </span>
                        <p className="text-sm font-semibold mt-0.5">{t(card.titleKey)}</p>
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    <div className="flex-1 space-y-1">
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wide">{t('联系账号')}</p>
                      {card.value ? (
                        <p className="text-lg font-bold font-mono tracking-wide">{card.value}</p>
                      ) : (
                        <p className="text-sm text-muted-foreground">{t('扫码加入')}</p>
                      )}
                    </div>
                      ...
                      {imgFailed ? (
                        ...
                        >
                          <Image
                            ...
                            onError={() => setFailedImages((prev) => new Set([...prev, card.key]))}
                          />
                        </button>
                      )}
                    </div>
      ...
      >
        <Card className="border-border/50 shadow-card">
          <CardContent className="pt-5 pb-5">
            <p className="text-sm font-semibold mb-3">{t('为了更快处理问题，联系时建议准备这些信息')}</p>
