...
function writeHomePageCache(content: string): void {
  localStorage.setItem(HOME_PAGE_CACHE_KEY, JSON.stringify({ content, timestamp: Date.now() }));
}
...
export default function HomePage() {
  ...
  useEffect(() => { displayContent(); }, [displayContent]);
  ...
  useEffect(() => {
    if (homePageContent.startsWith('https://') && iframeRef.current) {
      try {
        iframeRef.current.contentWindow?.postMessage({ themeMode: resolvedTheme }, '*');
  ...
  return (
    <div className="w-full overflow-x-hidden">
      ...
      <section className="relative min-h-[calc(100vh-var(--header-height))] flex items-center justify-center overflow-hidden border-b">
        ...
        {/* Background layers */}
        <div className="absolute inset-0 -z-10 grid-bg opacity-60" aria-hidden="true" />
        <div className="absolute inset-0 -z-10" aria-hidden="true">
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(90deg, transparent 0%, oklch(from var(--primary) l c h / 0.3) 50%, transparent 100%)',
            }}
          />
        </div>
        ...
        <div className="container max-w-4xl mx-auto px-4 py-20 flex flex-col items-center text-center gap-8">
          ...
          {/* CTAs */}
          <motion.div {...fadeUp(0.32)} className="flex gap-3 flex-wrap justify-center">
            <Link href="/console">
              <Button size="lg" className="rounded-full px-8 shadow-primary cursor-pointer">
          ...
          <motion.div {...fadeUp(0.38)} className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
            {[t('免费试用'), t('无需信用卡'), t('随时取消'), t('OpenAI 兼容')].map((item) => (
      ...
      <section className="py-20 border-b">
        <div className="container max-w-5xl mx-auto px-4">
          ...
          >
            ...
            <h2 className={`text-2xl sm:text-3xl font-bold ${isChinese ? 'tracking-wide' : ''}`}>
              {t('企业级 AI 接入基础设施')}
            </h2>
          ...
          >
            {FEATURES.map(({ icon: Icon, title, desc, iconColor, iconBg }) => (
              <motion.div key={title} variants={cardItem} className="feature-card group">
                <div className={`size-10 rounded-xl flex items-center justify-center mb-4 ${iconBg} transition-transform duration-200 group-hover:scale-110`}>
        ...
        <div className="container max-w-4xl mx-auto px-4">
          ...
          >
            ...
            <div className="flex gap-1 mb-0 bg-muted rounded-t-xl border border-b-0 border-border/60 p-1 w-fit">
              {([['after', t('接入网关')], ['before', t('原始代码')]] as const).map(([tab, label]) => (
                <button
                  key={tab}
            ...
            <div className="code-terminal">
              ...
              <div className="flex items-center gap-1.5 px-4 py-3 border-b border-white/[0.06]">
                ...
                <span className="ml-2 text-xs text-white/30 font-mono">python</span>
                {codeTab === 'after' && (
              ...
              <AnimatePresence mode="wait">
                <motion.pre
                  ...
                  aria-label={codeTab === 'after' ? t('接入网关代码示例') : t('原始代码示例')}
                >
        ...
        <div className="container max-w-3xl mx-auto px-4">
          ...
          >
            ...
            <div
              ...
              style={{
                background: 'radial-gradient(ellipse 80% 60% at 50% 50%, oklch(from var(--primary) l c h / 0.08) 0%, transparent 70%)',
              }}
              aria-hidden="true"
            />
            <div
              className="absolute top-0 left-0 right-0 h-px"
              style={{ background: 'linear-gradient(90deg, transparent, oklch(from var(--primary) l c h / 0.5), transparent)' }}
              aria-hidden="true"
            />

            <h2 className={`text-2xl sm:text-3xl font-bold mb-4 ${isChinese ? 'tracking-wide' : ''}`}>
              {t('立即开始使用')}
            </h2>
            ...
            <div className="flex gap-3 justify-center flex-wrap">
              <Link href="/console">
                <Button size="lg" className="rounded-full px-10 shadow-primary cursor-pointer">
                  {t('免费开始')}
