...
export default function StatusPage() {
  ...
  const loadAbout = async (force = false) => {
    ...
    try {
      ...
      const hasFreshCache =
        cached && Date.now() - cached.timestamp <= ABOUT_CACHE_TTL;
    ...
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-8 space-y-6">
      ...
      <motion.div
        ...
        <Button
          ...
          <RefreshCw
            className={`size-3.5 transition-transform duration-300 ${loading ? "animate-spin" : ""}`}
          />
          {t("刷新")}
        </Button>
      </motion.div>
          ...
          <Card className="shadow-card">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground leading-relaxed">
                {t("本系统基于")}{" "}
                <strong className="text-foreground">nbility</strong>{" "}
                {t(
                  "构建，是一个统一的 AI API 网关，支持 40+ 上游 AI 服务提供商。",
                )}
              </p>
              <p className="text-xs text-muted-foreground">
                {t(
                  "可在管理员设置页面配置关于内容，支持 Markdown 及外链 iframe。",
                )}
              </p>
