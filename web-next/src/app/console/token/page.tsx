  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <Button
        ...
        title={revealed ? t("隐藏") : t("查看完整 Key")}
      ...
      <Button
        ...
        className="size-7 text-muted-foreground hover:text-foreground shrink-0"
        title={t("复制")}
        onClick={fullKey ? handleCopy : handleCopyFull}>
...
function tokenToForm(t: Token): TokenFormData {
  return {
    ...
    expiredTime:
      t.expired_time > 0
        ? new Date(t.expired_time * 1000).toISOString().slice(0, 16)
        : "",
    group: t.group ?? "",
  };
}
...
function TokenDialog({
  ...
  useEffect(() => {
    if (open) setForm(editToken ? tokenToForm(editToken) : emptyForm());
  }, [open, editToken]);
  ...
  const set = <K extends keyof TokenFormData>(k: K, v: TokenFormData[K]) =>
    setForm((p) => ({ ...p, [k]: v }));

  const handleSubmit = async () => {
    if (!form.name.trim()) {
      toast.error(t("请输入令牌名称"));
      return;
    }
    setLoading(true);
    try {
      const body: Record<string, unknown> = {
        ...
        expired_time: form.expiredTime
          ? Math.floor(new Date(form.expiredTime).getTime() / 1000)
          : -1,
        ...(form.group ? { group: form.group } : {}),
      };
      if (isEdit) body.id = editToken!.id;
      const res = isEdit
        ? await API.put("/api/token/", body)
        : await API.post("/api/token/", body);
      ...
      if (data.success) {
        ...
        toast.error(data.message || (isEdit ? t("修改失败") : t("创建失败")));
      }
    } catch {
      toast.error(isEdit ? t("修改失败") : t("创建失败"));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="space-y-5 px-6 py-5">
          <div className="space-y-1.5">
            <Label className="text-sm">
              {t("令牌名称")} <span className="text-destructive">*</span>
            </Label>
            <Input
              value={form.name}
          ...
          <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {selectedGroup.billingLabel && (
                          <Badge
                            ...
                            className={cn(
                              ...
                              getGroupBillingBadgeClass(
                                selectedGroup.billingType,
                              ),
                            )}>
                            {selectedGroup.billingLabel}
                          </Badge>
                        ...
                        {selectedGroup.ratio !== undefined && (
                          <span className="text-xs font-medium text-muted-foreground">
                            {typeof selectedGroup.ratio === "string"
                              ? selectedGroup.ratio
                              : `×${selectedGroup.ratio}`}
                          </span>
                        )}
                      ...
                      <div className="flex items-center gap-2 flex-wrap pr-5">
                        ...
                        {g.billingLabel && (
                          <Badge
                            ...
                            className={cn(
                              "h-5 border px-1.5 text-[10px]",
                              getGroupBillingBadgeClass(g.billingType),
                            )}>
                            {g.billingLabel}
                          </Badge>
                        )}
            ...
            <p className="text-xs leading-5 text-muted-foreground">
              {t(
                "不选则继承账户默认分组；分组标签已标明订阅、按量或组合计费类型",
              )}
            </p>
          </div>
          <div className="flex items-center justify-between rounded-lg border border-border/60 bg-muted/30 px-3 py-2.5">
            ...
            <Switch
              ...
              onCheckedChange={(v: boolean) => set("unlimited", v)}
            />
          </div>
          {!form.unlimited && (
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                set("expiredTime", e.target.value)
              }
            />
          </div>
        </div>
...
function ImportConfigSheet({
  ...
  const providerName = `${status?.system_name || "nbility"} (${token?.name || ""})`;
  ...
  const fetchKeyAndImport = async (app: "claude" | "codex") => {
    ...
    try {
      ...
      const fullKey = data.data.key.startsWith("sk-")
        ? data.data.key
        : `sk-${data.data.key}`;
      const deepLink =
        app === "claude"
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full max-w-[95vw] sm:w-[360px] sm:max-w-none md:w-[420px]">
        <SheetHeader>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                  {t(
                    "配置 ANTHROPIC_AUTH_TOKEN 和 ANTHROPIC_BASE_URL，支持所有 Claude 模型",
                  )}
                </p>
...
function TokensContent() {
  ...
  const [confirmTarget, setConfirmTarget] = useState<"single" | "batch">(
    "single",
  );
  ...
  const allSelected =
    ...
    filteredTokens.every((tk) => selected.has(tk.id));
  const someSelected =
    filteredTokens.some((tk) => selected.has(tk.id)) && !allSelected;
  ...
  const handleToggleStatus = async (token: Token) => {
    ...
    try {
      ...
      if (data.success) {
        setTokens((prev) =>
          prev.map((tk) =>
            tk.id === token.id ? { ...tk, status: newStatus } : tk,
          ),
        );
    ...
    <div className="space-y-8 pb-10">
        ...
        <div className="flex flex-col gap-5 px-5 py-5 sm:px-6 sm:py-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1.5 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t(
                    "创建和管理 API 访问密钥，建议按业务、环境或分组拆分，提升权限隔离与计费可追踪性。",
                  )}
                </p>
              </div>
            </div>
            <div className="mt-5 flex flex-wrap items-center gap-2.5 text-xs text-muted-foreground">
              <span className="rounded-full border border-border/70 bg-background/80 px-3 py-1.5">
                {t("总计 {{count}} 个令牌", { count: tokens.length })}
              </span>
              <span className="rounded-full border border-border/70 bg-background/80 px-3 py-1.5">
                {t("启用 {{count}} 个", { count: activeCount })}
              </span>
              <span className="rounded-full border border-border/70 bg-background/80 px-3 py-1.5">
                {t("无限额度 {{count}} 个", { count: unlimitedCount })}
              </span>
            </div>
          </div>
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:min-w-[220px]">
            <Button
              variant="outline"
              size="sm"
              className="h-10 gap-1.5 rounded-2xl bg-background/80"
            ...
            <Button
              size="sm"
              className="h-11 gap-1.5 rounded-2xl shadow-primary/20 shadow-lg"
              onClick={openCreate}>
      ...
      <motion.div
        ...
        <StatFilterCard
          ...
          iconBg="bg-primary/10"
          iconColor="text-primary"
          active={statusFilter === "unlimited"}
          onClick={() => toggleCardFilter("unlimited")}
        />
      </motion.div>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          ...
          {selectedInView.length > 0 ? (
            ...
            <div className="lg:ml-auto flex items-center gap-2 text-xs text-muted-foreground">
              <span>{t("当前结果")}</span>
              <span className="rounded-full border border-border/70 bg-background/80 px-2.5 py-1 font-medium text-foreground">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {t("查看密钥状态、额度剩余、过期时间以及快捷导入配置。")}
                </p>
              </div>
              <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                <span className="rounded-full border border-border/70 bg-background/80 px-2.5 py-1">
                  {t("总数 {{count}}", { count: tokens.length })}
                </span>
                <span className="rounded-full border border-border/70 bg-background/80 px-2.5 py-1">
                  {t("筛选后 {{count}}", { count: filteredTokens.length })}
                </span>
              </div>
            </div>
          </div>
                        ...
                        <TableCell className="text-sm text-right">
                          {token.unlimited_quota ? (
                            ...
                            <div>
                              <div
                                className={cn(
                                  token.remain_quota <= 0 && "text-destructive",
                                )}>
                              ...
                              {status?.quota_display_type !== "TOKENS" &&
                                token.remain_quota > 0 && (
                                  <div className="text-xs text-muted-foreground font-mono">
                                    {formatTokensCompact(token.remain_quota)}
                                  </div>
                                )}
                            </div>
                          )}
                          ...
                          {status?.quota_display_type !== "TOKENS" &&
                            token.used_quota > 0 && (
                              <div className="text-xs font-mono">
                                {formatTokensCompact(token.used_quota)}
                              </div>
                            )}
                        </TableCell>
                            ...
                            <DropdownMenuContent align="end" className="w-44">
                              ...
                              <DropdownMenuItem
                                ...
                                {token.status === 1 ? (
                                  ...
                                  <>
                                    <Activity className="size-4 mr-2 text-success" />
                                    {t("启用")}
                                  </>
                                )}
                              </DropdownMenuItem>
      ...
      <ConfirmDialog
        ...
        title={
          confirmTarget === "batch"
            ? `${t("批量删除")} ${selectedInView.length} ${t("个令牌")}`
            : t("删除令牌")
        }
