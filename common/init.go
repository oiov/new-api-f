...
func InitEnv() {
	...
	SkipAutoMigrate = GetEnvOrDefaultBool("SKIP_AUTO_MIGRATE", false)
	...
	TokenTestRateLimitEnable = GetEnvOrDefaultBool("TOKEN_TEST_RATE_LIMIT_ENABLE", true)
	TokenTestRateLimitNum = GetEnvOrDefault("TOKEN_TEST_RATE_LIMIT", 60)
	TokenTestRateLimitDuration = int64(GetEnvOrDefault("TOKEN_TEST_RATE_LIMIT_DURATION", 20*60))
...
func initConstantEnv() {
	...
	// AnonymousRequestBodyLimitKB 未认证关键路由(登录/注册/回调等)的请求体上限(KB)，默认 512；设为 0 关闭
	...
	constant.TaskPricePatches = buildTaskPricePatches(GetEnvOrDefaultString("TASK_PRICE_PATCH", ""))
...
func buildTaskPricePatches(envValue string) []string {
	patches := []string{"seedance-2-cheap"}
