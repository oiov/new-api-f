...
func SetApiRouter(router *gin.Engine) {
	...
	// 未认证关键路由(登录/注册/OAuth 绑定/支付回调等)的请求体大小限制,防匿名端点超大 body 攻击
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.GET("/affiliate/admin/commissions", middleware.AdminAuth(), middleware.PermissionAuth(common.PermissionPointUserView), controller.AdminGetAffiliateCommissions)
				selfRoute.POST("/affiliate/admin/commissions/:id/approve", middleware.AdminAuth(), middleware.PermissionAuth(common.PermissionPointUserEdit), controller.AdminApproveAffiliateCommission)
				selfRoute.POST("/affiliate/admin/commissions/:id/reject", middleware.AdminAuth(), middleware.PermissionAuth(common.PermissionPointUserEdit), controller.AdminRejectAffiliateCommission)
				...
				selfRoute.GET("/self/notifications", controller.ListSelfSiteNotifications)
				selfRoute.GET("/self/notifications/unread_count", controller.GetSelfSiteNotificationUnreadCount)
				selfRoute.POST("/self/notifications/:id/read", controller.MarkSelfSiteNotificationRead)
				selfRoute.POST("/self/notifications/read_all", controller.MarkAllSelfSiteNotificationsRead)
		...
		activityLotteryAdminRoute.Use(middleware.AdminAuth())
		{
			...
			activityLotteryAdminRoute.GET("/auto_jobs", middleware.PermissionAuth(common.PermissionPointActivityAdminView), controller.AdminListActivityLotteryAutoJobs)
			activityLotteryAdminRoute.GET("/auto_jobs/:id", middleware.PermissionAuth(common.PermissionPointActivityAdminView), controller.AdminGetActivityLotteryAutoJob)
		...
		tokenRoute.Use(middleware.UserAuth())
		{
			...
			tokenRoute.GET("/business-groups", controller.GetBusinessGroups)
